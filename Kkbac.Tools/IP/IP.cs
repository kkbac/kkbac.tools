...
namespace Kkbac.Tools.IP
{
    ...
    public class CZIP
    {
        ...
        //public IP()
        //{
        //    _dbFilePath = System.Web.HttpContext.Current.Server.MapPath("/files/ip/ip.dat");
        //    bFilePathInitialized = false;
        //    SetDbFilePath(_dbFilePath);
        //}

        public CZIP(string dbFilePath)
        {
            bFilePathInitialized = false;
        ...
        /// <returns></returns>
        public string GetIP(string IPValue,
            out string country,
            out string area
        )
        {
            ...
            area = "";
            var ipaddress = "";
            GetAddressWithIP(IPValue, out country, out area);
            country = country.Replace("CZ88.NET", "");
            //.Replace("IANA", "国际互联网代理成员管理局(IANA)");
            area = area.Replace("CZ88.NET", "");
            //.Replace("IANA", "国际互联网代理成员管理局(IANA)");
            area = area.Replace("对方和您在同一内部网", "");

            ipaddress = country + " " + area;
            return ipaddress;
        }
        ...
        )
        {
            ...
            while (true)
            {
                ...
                if (ip >= Search_Set.IpSet && ip <= Search_Set.IpEnd)
                    return ReadAddressInfoAtOffset(Search_Set.Offset, out country, out area);


                //判断IP是否在区间尾内
                if (ip >= Search_End.IpSet && ip <= Search_End.IpEnd)
                    return ReadAddressInfoAtOffset(Search_End.Offset, out country, out area);
                ...
                if (ip >= Search_Mid.IpSet && ip <= Search_Mid.IpEnd)
                    return ReadAddressInfoAtOffset(Search_Mid.Offset, out country, out area);
